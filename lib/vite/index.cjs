...
function qwikCityPlugin(userOpts) {
  ...
  const plugin = {
    ...
    async config(viteConfig) {
      const hasCloudflarePlugin = (viteConfig.plugins ?? []).flat().some((p) => (p == null ? void 0 : p.name) === "vite-plugin-cloudflare");
    ...
    async configResolved(config) {
      ...
      if ((userOpts == null ? void 0 : userOpts.devSsrServer) === void 0) {
    ...
    configureServer(server) {
      return () => {
        ...
        if ((userOpts == null ? void 0 : userOpts.devSsrServer) === false) {
          return;
        }
