...
export function qwikVite(qwikViteOpts: QwikVitePluginOptions = {}): any {
  ...
  // Set to true when @cloudflare/vite-plugin is detected, enabling Vite 7 Environment API mode.
  ...
  const vitePluginPre: P<QwikVitePluginApi> = {
    ...
    async config(viteConfig, viteEnv) {
      ...
      // Detect @cloudflare/vite-plugin to enable Vite 7 Environment API mode.
      // When present, qwikVite delegates SSR build management to the cloudflare plugin
      // and scopes client build config to environments.client to avoid contamination.
      hasCloudflarePlugin = ((viteConfig.plugins ?? []) as any[])
        .flat()
        .some((p: any) => p?.name === 'vite-plugin-cloudflare');

      let target: QwikBuildTarget;
      if (hasCloudflarePlugin) {
        // Environment API mode: qwikVite handles client config only.
        // The cloudflare plugin manages the SSR/Worker environment independently.
        target = 'client';
      ...
      if (!qwikViteOpts.csr) {
        ...
        const rollupOutput = await normalizeRollupOutputOptions(
          qwikPlugin,
          viteConfig.build?.rollupOptions?.output,
          useAssetsDir,
          buildOutputDir
        );
        const rollupOnwarn = (warning: any, warn: any) => {
        ...
        if (hasCloudflarePlugin) {
          // Vite 7 Environment API mode: scope client build config to environments.client
          // to prevent contaminating the SSR/Worker environment managed by @cloudflare/vite-plugin.
          // When cloudflare plugin is present, target is always 'client' so no SSR/lib/test paths needed.
          const clientBuildInput = isClientDevOnly && !opts.csr ? clientDevInput : opts.input;
          updatedViteConfig.environments = {
            client: {
              build: {
                cssCodeSplit: false,
                outDir: buildOutputDir,
                // Prevent the client build from cleaning dist/ which would delete
                // dist/ssr/ (the Worker bundle) generated by the SSR environment build.
                emptyOutDir: false,
                rollupOptions: {
            ...
            ssr: {
              // @builder.io/qwik/server imports @qwik-client-manifest (virtual module)
              // which esbuild pre-optimization cannot resolve. Exclude from pre-bundling
              // so Vite's module pipeline handles it via qwikVite's resolveId hook.
              optimizeDeps: {
                exclude: [QWIK_CORE_ID, QWIK_CORE_SERVER],
              },
            },
          };
        } else {
          // Legacy mode: global build config for traditional adapter approach (backward compat).
    ...
    resolveId(id, importer, resolveIdOpts) {
      ...
      // Environment API: override browser-only Qwik modules in SSR/Worker environment.
      // workerd is not a browser: isBrowser must be false, and preloader uses `document`.
      const envName = (this as any).environment?.name;
      if (envName && envName !== 'client') {
        if (id === QWIK_BUILD_ID) {
          return '\0qwik-build-server-env';
        }
        if (id === '@builder.io/qwik/preloader') {
          return '\0qwik-preloader-server-env';
        }
      }
    ...
    load(id, loadOpts) {
      // Handle virtual modules for SSR environment overrides before the '\0' early return.
      if (id === '\0qwik-build-server-env') {
        return [
          '// @builder.io/qwik/build override for SSR/Worker environment (not a browser)',
          `export const isBrowser = false;`,
          `export const isServer = true;`,
          `export const isDev = ${viteCommand === 'serve'};`,
        ].join('\n');
      }
      if (id === '\0qwik-preloader-server-env') {
        return [
          '// @builder.io/qwik/preloader noop for SSR/Worker environment (no document)',
          'export const p = () => {};',
          'export const g = () => {};',
          'export const h = () => {};',
          'export const l = () => {};',
  ...
  const vitePluginPost: VitePlugin<never> = {
    ...
    generateBundle: {
      ...
      async handler(_, rollupBundle) {
        ...
        // Environment API: skip client manifest generation for non-client environments.
        // When cloudflare plugin is present, target is always 'client' but the SSR
        // environment also triggers this hook — guard by environment name.
        const envName = (this as any).environment?.name;
        if (envName && envName !== 'client') {
          return;
        }
