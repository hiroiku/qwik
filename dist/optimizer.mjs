...
function qwikVite(qwikViteOpts = {}) {
  ...
  const vitePluginPre = {
    ...
    async config(viteConfig, viteEnv) {
      ...
      hasCloudflarePlugin = (viteConfig.plugins ?? []).flat().some(p => "vite-plugin-cloudflare" === p?.name);
      let target;
      target = hasCloudflarePlugin ? "client" : viteConfig.build?.ssr || "ssr" === viteEnv.mode ? "ssr" : "lib" === viteEnv.mode ? "lib" : "test" === viteEnv.mode ? "test" : "client";
      let buildMode;
      ...
      if (!qwikViteOpts.csr) {
        ...
        const rollupOutput = await normalizeRollupOutputOptions(qwikPlugin, viteConfig.build?.rollupOptions?.output, useAssetsDir, buildOutputDir);
