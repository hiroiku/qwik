  ...
  function qwikVite(qwikViteOpts = {}) {
    ...
    const vitePluginPre = {
      ...
      async config(viteConfig, viteEnv) {
        ...
        hasCloudflarePlugin = (viteConfig.plugins ?? []).flat().some(p => "vite-plugin-cloudflare" === (null == p ? void 0 : p.name));
        let target;
        target = hasCloudflarePlugin ? "client" : (null == (_a = viteConfig.build) ? void 0 : _a.ssr) || "ssr" === viteEnv.mode ? "ssr" : "lib" === viteEnv.mode ? "lib" : "test" === viteEnv.mode ? "test" : "client";
        let buildMode;
        ...
        if (!qwikViteOpts.csr) {
          ...
          const rollupOutput = await normalizeRollupOutputOptions(qwikPlugin, null == (_v = null == (_u = viteConfig.build) ? void 0 : _u.rollupOptions) ? void 0 : _v.output, useAssetsDir, buildOutputDir);
          const rollupOnwarn = (warning, warn) => {
            if ("typescript" === warning.plugin && warning.message.includes("outputToFilesystem")) {
              return;
            }
            origOnwarn ? origOnwarn(warning, warn) : warn(warning);
      ...
      resolveId(id, importer, resolveIdOpts) {
        ...
        if (envName && "client" !== envName) {
          if (id === QWIK_BUILD_ID) {
            return "\0qwik-build-server-env";
          }
          if ("@builder.io/qwik/preloader" === id) {
      ...
      load(id, loadOpts) {
        if ("\0qwik-build-server-env" === id) {
          return [ "// @builder.io/qwik/build override for SSR/Worker environment (not a browser)", "export const isBrowser = false;", "export const isServer = true;", `export const isDev = ${"serve" === viteCommand};` ].join("\n");
        }
        if ("\0qwik-preloader-server-env" === id) {
          return [ "// @builder.io/qwik/preloader noop for SSR/Worker environment (no document)", "export const p = () => {};", "export const g = () => {};", "export const h = () => {};", "export const l = () => {};" ].join("\n");
        }
